defmodule Adventofcode24.DayThree do
  ...
  def run() do
    ...
    sum = Regex.scan(~r/mul\((\d+),(\d+)\)/, data, capture: :all_but_first)
    |> Enum.map(fn([first | [second | _]] = _x) -> String.to_integer(first) * String.to_integer(second) end)
    |> Enum.sum()
  ...
  def run2() do
    ...
    instructions = Regex.scan(~r/mul\(\d{1,3},\d{1,3}\)|do\(\)|don't\(\)/, data)
